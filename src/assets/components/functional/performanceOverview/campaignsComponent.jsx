...
const CampaignsComponent = (props, ref) => {
    ...
    const selectedBrand = searchParams.get("brand") || "continental Cosmetics";
    ...
    const clearCampaignCaches = () => {
        try {
            ...
            for (let i = 0; i < localStorage.length; i++) {
                ...
                if (key && key.includes('/continental/campaign')) {
    ...
    const getCampaignsData = async (forceRefresh = false) => {
        ...
        try {
            ...
            let url = `https://react-api-script.onrender.com/continental/campaign?start_date=${startDate}&end_date=${endDate}&platform=${operator}${ts}`;
    ...
    const handleCampaignClick = async (campaignName, campaignId) => {
        try {
            ...
            const startDate = formatDate(dateRange[0].startDate);
            const endDate = formatDate(dateRange[0].endDate);
            let url = `https://react-api-script.onrender.com/continental/campaign_graph?start_date=${formatDate(startDate)}&end_date=${formatDate(endDate)}&platform=${operator}&campaign_id=${campaignId}`;
            if (selectedBrand && typeof selectedBrand === "string") {
                url += `&brand_name=${encodeURIComponent(selectedBrand)}`;
            }
            const cacheKey = `cache:GET:${url}`;
   ...
   const confirmStatusChange = async (campaignId, newStatus, adType) => {
    try {
        ...
        const playPauseUrl = `https://react-api-script.onrender.com/continental/play-pause`;
